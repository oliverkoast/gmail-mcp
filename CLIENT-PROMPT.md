...
per-account steps so the prompt stays short. Replace `<REPO_URL>` once this
repo is pushed somewhere the client can reach (private GitHub + invite, or a
zip you send them).
...
1. Clone <REPO_URL> into a folder in my home directory (or unzip the archive
